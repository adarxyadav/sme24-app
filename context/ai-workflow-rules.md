...
Build SME24 incrementally with a spec-driven workflow. Before
...
1. **Orient**: Read project overview, architecture, pipeline rules,
...
- Work on one feature unit at a time. Example: expert matching
  work may touch `components/matching/`, `app/page.tsx`, and
  related docs, but should not add persistence.
...
- Do not combine unrelated system boundaries in a single step.
  ...
  unless the feature spec explicitly requires both.
...
- Trigger.dev or AI pipeline behavior → `context/pipeline-rules.md`
